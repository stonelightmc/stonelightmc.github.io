...
async function loadLiveStatus(force = false) {
  ...
  try {
    ...
    if (data.online) {
      ...
      renderOnlinePlayersFromPlugin(online);
    } else {
...
function getPluginOnlinePlayerNames() {
  const players = statsData?.onlinePlayers || statsData?.server?.onlinePlayers || [];
  return (Array.isArray(players) ? players : [])
    .map(normalizePlayerName)
    .filter(Boolean);
}

function renderOnlinePlayersFromPlugin(onlineCount = null) {
  const box = $('online-players');
  if (!box) return;

  const names = getPluginOnlinePlayerNames();

  if (names.length === 0) {
    if (Number(onlineCount || 0) > 0) {
      box.innerHTML = `<p class="muted">Зараз онлайн: ${onlineCount}. Очікуємо список гравців від StoneLightStats.</p>`;
    } else {
...
function renderOnlinePlayers(list, onlineCount) {
  // Backward-compatible wrapper: online names now come from StoneLightStats JSON,
  // not from mcsrvstat, because mcsrvstat often has only the player count.
  renderOnlinePlayersFromPlugin(onlineCount);
}
...
async function loadStatsJson() {
  try {
    ...
    renderOnlinePlayersFromPlugin(Number($('metric-online')?.textContent || 0));
...
function updateServerMetricsFromJson(data) {
  ...
  setText('metric-core', formatCoreLabel(server));
}

function formatCoreLabel(server) {
  const coreName = server.coreName || 'Paper';

  const serverVersion = String(server.serverVersion || '');
  const mcMatch = serverVersion.match(/\(MC:\s*([^\)]+)\)/i);
  if (mcMatch && mcMatch[1]) {
    return `${coreName} ${mcMatch[1].trim()}`;
  }

  const bukkitVersion = String(server.bukkitVersion || '');
  if (bukkitVersion) {
    return `${coreName} ${bukkitVersion.replace(/\.build.*$/i, '').trim()}`;
  }

  if (server.coreVersion) {
    return `${coreName} ${server.coreVersion}`;
  }
